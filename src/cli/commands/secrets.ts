...
export async function runSecretsSet(
  ...
  if (!appId) {
    console.error('用法: lcb secrets set --app-id <id>');
    process.exit(1);
  }
...
export async function runSecretsRemove(
  ...
  if (!appId) {
    console.error('用法: lcb secrets remove --app-id <id>');
    process.exit(1);
  }
